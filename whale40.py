...
def yahoo_prices(sym: str) -> dict[str, float]:
    """실패 시 사유를 yahoo_prices.last_error 에 남긴다."""
    ...
    for host in ("query1", "query2"):
        ...
        try:
            ...
            why = f"JSON 아님: {r.text[:90]!r}"
            continue
        chart = body.get("chart") or {}
        if chart.get("error"):
            why = f"야후 오류: {chart['error']}"
            continue
        ...
        try:
            ...
            why = f"구조 해석 실패: {type(e).__name__}"
        if px:
            break
    if not px and why:
        yahoo_prices.last_error = why
    if not px and b:
        px = b["px"]
...
def inspect():
    ...
    if not px:
        log(f"    [debug] 사유: {getattr(yahoo_prices, 'last_error', '알 수 없음')}")
        r = get("https://query1.finance.yahoo.com/v8/finance/chart/AAPL?range=5d&interval=1d",
                tries=1, quiet=True)
        log(f"    [debug] 단순 요청: {'응답 받음 ' + str(len(r.content)) + 'bytes' if r else '실패'}")
        if r:
            log(f"    [debug] 앞부분: {r.text[:200]!r}")
