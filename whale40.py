...
def render(inst, ppl, hits, delta, agg, hist):
    today = date.today().isoformat()
    L = [f"# 고래 40 · {today}", "",
...
def make_pdf(path, inst, ppl, hits, delta, agg, hist):
...
def main():
    ...
    md = render(inst, ppl, hits, delta, agg, hist)
    today = date.today().isoformat()
    ...
    pdf = None
    try:
        pdf = make_pdf(REPORT / f"{today}.pdf", inst, ppl, hits, delta, agg, hist)
        if pdf:
