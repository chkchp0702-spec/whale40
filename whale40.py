...
def label(sym: str, issuer: str = "") -> str:
...
def rank_group(entries, label):
    ...
    for path, s in snaps_by.items():
        ...
        rows.append({**meta[path], "ret_1y": round(r * 100, 2), "coverage": round(cov, 2),
                     ...
                     "top": [label(k, last["names"].get(k, "")) for k, _ in
...
def render(inst, ppl, hits, delta):
    ...
    for h in hits:
        ...
        for x in h["new"][:12]:
            L.append(f"- ✨ 신규 **{label(x['sym'], x['issuer'])}** · {x['shares']:,.0f}주 · {money(x['value'])}")
        for x in h["added"][:12]:
            L.append(f"- ➕ 추가 **{label(x['sym'], x['issuer'])}** · {money(x['value'])} ({x['pct']:+.0f}%)")
...
def summary(inst, ppl, hits, delta):
    ...
    if hits:
        ...
        L.append("")
        for h in hits[:6]:
            names = ", ".join(label(x["sym"], x["issuer"]) for x in h["new"][:4])
...
def inspect():
    ...
    if f:
        ...
        if qs:
            ...
            for h in hs[:5]:
                log(f"    {label(h['sym'], h['issuer'])[:26]:26} {h['value']:>14,.0f}천$ {h['shares']:>14,.0f}주")
