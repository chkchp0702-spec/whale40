...
def make_print_html(path, inst, ppl, agg, hist, details, items, prev, pc, Q, disc, conv,
    ...
    nxt, dday = next_13f_deadline()
    hero = hero_pick(Q, mov, list(clus), list(cost), list(warn), idx)
    ...
    def sym_cell(r, names=True, n=2):
        nm = _names(r, n) if names else ""
        return (f'<div class="sym"><b>{_h(r["sym"])}</b> <span>{_h(label_only(r["sym"], r.get("name", ""))[:16])}</span></div>'
                + (f'<div class="who">{_h(nm)}</div>' if nm else ""))
    ...
    if hero:
        ...
        who = _names(r, 4)
        hero_html = (f'<div class="hero" style="border-left-color:{c}"><div class="tag" style="color:{c}">오늘 이것 하나 · {_h(zt)} · {_h(act)}</div>'
                     ...
                     + '<ul>' + "".join(f"<li>{_h(x)}</li>" for x in hero["why"]) + "</ul>"
                     + '<div class="facts">' + "".join(f'<span><i>{_h(a)}</i><em class="{cl}">{_h(b)}</em></span>' for a, b, cl in facts) + "</div>"
                     + f'<p class="zwhy">{_h(zd)}</p></div>')
    else:
    ...
    legend = ''.join(f'<div class="lg" style="border-top-color:{ZC[k]}"><b style="color:{ZC[k]}">{_h(ZONE[k][2])} · {_h(ZONE[k][0])}</b><p>{_h(d)}</p></div>'
                     for k, d in (("contra", "고래는 사는데 주가는 내렸다. 지금 사면 고래와 비슷하거나 더 싼 값."),
                                  ("chase", "고래도 이미 오른 뒤 탔다. 따라 사면 고래보다 훨씬 비싸게 산다."),
                                  ("profit", "오른 뒤 고래가 나갔다. 보유 중이면 차익 실현 시점을 따져볼 것."),
                                  ("flee", "내리는데 고래도 팔았다. 새로 들어갈 이유가 약하다.")))
    p1 = (masthead(1, "오늘 무엇을 볼까", "고래 40곳의 매집·이탈을 주가·거래량과 대조해 행동으로 옮길 것만 골랐습니다")
          ...
          + h2("오늘의 한 줄", "히어로 카드 외에 눈여겨볼 것") + f'<ul class="heads">{"".join(hl)}</ul>')
    ...
    p3 = (masthead(3, "무엇을 사고, 무엇을 점검하나", "이름이 붙어 있습니다. '몇 곳'보다 '누가'가 행동으로 이어집니다")
          + '<div class="two">'
          + '<div>' + h2("동시 편입", "같은 분기에 여러 곳이 한꺼번에 신규로 담은 종목. 상장 1년 미만은 뺐습니다") + cl_t + '</div>'
          + '<div>' + h2("고래 평단 대비 현재가", "분기별 순매수 × 그 분기 평균가로 추정. 음수면 고래보다 싸다. ●는 근거 분기 수") + co_t + '</div>'
          + '</div><div class="two">'
          + '<div>' + h2("전일 신호", "어제 3% 이상 움직였거나 거래량이 20일 평균의 2배") + mv_t + '</div>'
          + '<div>' + h2("보유 중이면 점검", "2분기 이상 연속 이탈 + 성적 좋은 곳이 먼저 빠짐", "stop") + wn_t + '</div>'
          + '</div>' + h2("곧 실적 발표", "고래 5곳 이상 든 종목 중 열흘 안에 실적이 잡힌 것 — 행동 시점이 여기 걸립니다") + er_t)
...
.hero{border:1px solid var(--line);border-left:5px solid var(--go);border-radius:3mm;padding:4.5mm 6mm 4mm;background:#fff}
...
.hero ul{margin:0 0 3mm;padding-left:5mm}.hero li{font-size:10.5pt;margin:.6mm 0}
...
.live{display:flex;flex-direction:column;gap:1.5mm}
.lrow{display:grid;grid-template-columns:auto 44mm 1fr auto;align-items:center;gap:3mm;padding:2mm 3mm;border:1px solid var(--line);border-radius:2.5mm;font-size:9.5pt}
...
.t .sym b{font-weight:800}.t .sym span{color:var(--mut);font-size:8.6pt}
.t.tight td{padding:1.3mm 1.5mm}
td.conf{letter-spacing:.06em;color:var(--go)}
.bts{display:grid;grid-template-columns:repeat(4,1fr);gap:4mm}
...
.lg{border-top:3px solid;background:var(--paper);border-radius:2mm;padding:1.6mm 2.5mm}
.lg b{font-size:8.6pt;display:block}.lg p{margin:.6mm 0 0;font-size:7.9pt;color:var(--mut);line-height:1.35}
    ...
    try:
        ...
        try:
            ph = make_print_html(REPORT / f"{today}-print.html", inst, ppl, U, hist, details, items, prev,
                                 pc, Q, disc, conv, mov, secs, idx, live, clus, cost, warn, earn, bt)
