...
def sec_get(url, timeout=40):
    ...
    try:
        ...
        if r.status_code == 200:
            return r
        log(f"    SEC HTTP {r.status_code}: {url[:70]}")
        return None
    except requests.RequestException as e:
...
def shares_outstanding(sym, tc=None):
    ...
    if e and e.get("asof", "") >= (date.today() - timedelta(days=90)).isoformat():
        return e.get("v")
    tc = tc or ticker_cik_map()
    cik = tc.get(sym.upper().replace(".", "-"))
    val = None
    if cik:
...
def earnings_soon(syms, days=10):
    ...
    today = date.today()
    if cache.get("_asof") != today.isoformat():
        ...
        for i in range(days + 1):
            ...
            try:
                ...
                if r.status_code != 200:
                    if i == 0:
                        log(f"  실적 캘린더 HTTP {r.status_code} — 나스닥이 이 IP를 막았을 수 있음")
                    continue
        ...
        log(f"  실적 일정 {len(cache) - 1}종목 (나스닥 캘린더)")
    out = {}
    for s in syms:
        e = cache.get(s)
...
def make_print_html(path, inst, ppl, agg, hist, details, items, prev, pc, Q, disc, conv,
    ...
    p3 = (masthead(3, "무엇을 사고, 무엇을 점검하나", "이름이 붙어 있습니다. '몇 곳'보다 '누가'가 행동으로 이어집니다")
          ...
          + '</div>' + h2("전일 신호", "고래 5곳 이상 보유 종목 중 어제 3% 이상 움직였거나 거래량이 20일 평균의 2배") + mv_t)
    ...
    if bt and bt.get("zones"):
        ...
        bench = f'같은 기간 S&P500 {bt["bench"]:+.1f}%' if bt.get("bench") is not None else ""
        bt_html = f'<div class="bts">{"".join(cells)}</div><p class="note">{_h(bt["quarter"])} 기준으로 구역을 나눈 뒤 지금까지의 수익률 · {_h(bench)} · 고래 5곳 이상, 순증감 절댓값 3 이상만</p>'
    else:
    ...
    p4 = (masthead(4, "구조와 검증", "누가 잘하고, 어디에 돈이 몰리고, 이 방법이 실제로 먹혔나")
          ...
          + '<p class="fine">가중 순증감 = 성적 좋은 펀드의 표를 더 무겁게 센 편입·편출 합계(1위 2.5표 ~ 하위 1.0표) · 동시 신규 = 같은 분기에 새로 담은 펀드 수 · 연속 이탈 = 보유 펀드 수가 몇 분기 연달아 줄었는지. 평단·수익률·비중은 13F에서 역산한 추정치이며 실제 체결가가 아닙니다. 유니버스: 고래 5곳 이상·상장 1년 이상·ETF·워런트·SPAC 제외. 출처 13f.info · SEC EDGAR · Yahoo Finance · 투자 자문이 아닙니다.</p>').replace("인덕스", "인덱스")
