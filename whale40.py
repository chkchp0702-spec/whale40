...
def cik_index() -> list[tuple[str, str, int]]:
    ...
    if not raw:
        ...
        r = get(SEC, LOOKUP_URL, tries=3, sleep=0.3)
        if not r:
            log("  ! CIK 목록 다운로드 실패")
            _INDEX = []
            return _INDEX
...
def parse_13f(cik, acc):
    ...
    r = get(SEC, f"{adir(cik, acc)}/{xs[0]}", quiet=True)
    if not r: return {}
...
def preflight() -> bool:
    ...
    ok = True
    r = get(SEC, "https://data.sec.gov/submissions/CIK0001067983.json", tries=2)
    if r:
    ...
    if not ok:
        log("")
        log("SEC가 이 서버 IP를 차단한 상태입니다.")
        log("SEC는 과도한 요청이나 잘못된 User-Agent를 감지하면 약 10분간 IP를 막습니다.")
        log("조치: 15분 이상 기다렸다가 다시 실행하세요. 재시도는 이미 자동으로 했습니다.")
        log("반복되면 SEC_USER_AGENT 를 'Hong Gildong hong@example.com' 형식으로 다시 저장하세요.")
    return ok
