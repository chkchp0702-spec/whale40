...
MIN_WEIGHT = 0.5        # 이 비중(%) 미만 포지션은 '한 표'로 세지 않는다
    ...
    if cache.get("_asof") != today.isoformat():
        ...
        for i in range(days + 1):
            ...
            if d.weekday() >= 5:
                continue
            try:
                r = requests.get(f"https://api.nasdaq.com/api/calendar/earnings?date={d.isoformat()}", headers=NDQ_UA, timeout=20)
                if r.status_code != 200:
                    continue
...
def hero_brief(h, big, zone_key):
    ...
    why = " · ".join(parts) or "—"
    act = {"contra": "고래와 비슷하거나 더 싼 값에 살 수 있는 구간. 관심 목록에 올리고 실적·13D 공시를 지켜볼 것.",
...
def make_print_html(path, inst, ppl, agg, hist, details, items, prev, pc, Q, disc, conv,
    ...
    if idx and idx["r1y"] is not None:
        ...
        kp.append(("고래 인덱스 · 1년", f'{idx["r1y"]:+.1f}%', f'S&P500 {idx["s1y"]:+.1f}% ({gap:+.1f}p)', _cls(idx["r1y"])))
...
def main():
    ...
    watch, seen_path = [], {}
    for r in [{"group": "기관", **r} for r in inst[:TOP_N]] + [{"group": "유명인", **r} for r in ppl[:TOP_N]]:
