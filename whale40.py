...
def holdings(fid: str, debug: bool = False) -> list[dict]:
    ...
    # (2) 표를 채우는 JSON
    rows = []
    d = get(f"{BASE}/data/13f/{fid}", quiet=True)
    if d:
        try:
            j = d.json()
            if debug:
                log(f"    [debug] data JSON 키: {list(j)[:6] if isinstance(j, dict) else type(j).__name__}")
            if isinstance(j, dict):
                for key in ("data", "rows", "aaData", "results"):
            ...
            if debug:
                log(f"    [debug] data 응답이 JSON 아님: {d.text[:120]!r}")
    elif debug:
        log("    [debug] /data/13f/ 응답 없음")
