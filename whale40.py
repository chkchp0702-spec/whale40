...
def hero_pick(Q, mov, clus, cost, warn, idx, big=()):
    ...
    pool.update({r["sym"]: (pool.get(r["sym"], ("", r))[0], r) for r in list(big) + clus + mov + cost + warn})
...
def make_mobile_html(path, inst, ppl, agg, hist, details, items, prev, pc, Q, disc, conv,
                     mov, secs, idx, live, clus, cost, warn, earn, bt, big=()):
    today = date.today().isoformat()
    wd = "월화수목금토일"[date.today().weekday()]
    nxt, dday = next_13f_deadline()
    hero = hero_pick(Q, mov, clus, cost, warn, idx, list(big))
...
def make_print_html(path, inst, ppl, agg, hist, details, items, prev, pc, Q, disc, conv,
    ...
    nxt, dday = next_13f_deadline()
    hero = hero_pick(Q, mov, list(clus), list(cost), list(warn), idx, list(big))
    ...
    kp.append(("실시간 신호", f'{len(live)}건', "Form 4 매수 · 13D/G · 최근 5영업일", "hot" if live else "mut"))
    ...
    p1 = (masthead(1, "오늘 무엇을 볼까", "고래 40곳의 매집·이탈을 주가·거래량과 대조해 행동으로 옮길 것만 골랐습니다")
          ...
          + f'<div class="live">{live_html}</div>'
    ...
    # ── 3쌍 행동 ──
    bg_t = table([("종목 · 누가 (비중)", ""), ("신규", "r"), ("고래 지분", "r"), ("1년", "r")],
                 [[(f'<div class="symrow"><div><div class="sym"><b>{_h(r["sym"])}</b> <span>{_h(label_only(r["sym"], r.get("name", ""))[:14])}</span></div>'
                   ...
                   (f'{r["big_n"]}곳', ""), (f'{r["own_pct"]:.1f}%' if r.get("own_pct") else "—", ""),
    ...
    p4 = (masthead(4, "구조와 검증", "누가 잘하고, 어디에 돈이 몰리고, 이 방법이 실제로 먹혔나")
          ...
          + f'<p class="fine">가중 순증감 = 성적 좋은 펀드의 표를 더 무겁게 센 편입·편출 합계(1위 2.5표 ~ 하위 1.0표) · 동시 신규 = 같은 분기에 새로 담은 펀드 수 · 연속 이탈 = 보유 펀드 수가 몇 분기 연달아 줄었는지. 평단·수익률·비중은 13F에서 역산한 추정치이며 실제 체결가가 아닙니다. 유니버스: 고래 {MIN_HOLD}곳 이상·상장 1년 이상·ETF·워런트·SPAC 제외. 출처 13f.info · SEC EDGAR · Yahoo Finance · 투자 자문이 아닙니다.</p>').replace("인덕스", "인덱스")
...
.kpi.up{border-top-color:var(--up)}.kpi.down{border-top-color:var(--down)}.kpi.hot{border-top-color:var(--hot)}.kpi.warn{border-top-color:var(--warn)}.kpi.mut{border-top-color:var(--stop)}
...
def top3_lines(Q, clus, cost, mov, warn, live, big=()):
...
def summary(inst, ppl, hits, delta, agg, prev=None, live=None, Q=None, clus=None,
            cost=None, mov=None, warn=None, big=None):
    prev = prev or {}
    L = [f"🐋 <b>고래 40 · {date.today().isoformat()}</b>", ""]
    t3 = top3_lines(Q, clus or [], cost or [], mov or [], warn or [], live or [], big or [])
    if t3:
        L += t3 + ["", "─────────", ""]
    if hits:
...
def main():
    ...
    try:
        ...
        log(f"  가격맥락 {len(pc)} · 사분면 {sum(len(v) for v in Q.values())} · 전일신호 {len(mov)} · "
            f"큰신규 {len(big)} · 동시편입 {len(clus)} · 평단 {len(cost)} · 경고 {len(warn)} · 실적 {len(earn)}"
        ...
        try:
            ph = make_print_html(REPORT / f"{today}-print.html", inst, ppl, U, hist, details, items, prev,
                                 pc, Q, disc, conv, mov, secs, idx, live, clus, cost, warn, earn, bt, big)
    ...
    send_telegram(summary(inst, ppl, hits, delta, agg, prev, live, locals().get("Q"),
                          ...
                          locals().get("warn"), locals().get("big")),
