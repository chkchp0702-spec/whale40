...
# SEC가 GitHub 서버 IP를 차단하므로, Cloudflare Worker를 중계로 둘 수 있다.
# SEC_PROXY 가 설정되면 모든 SEC 요청이 그 주소를 경유한다.
PROXY = os.environ.get("SEC_PROXY", "").strip().rstrip("/")
PROXY_KEY = os.environ.get("SEC_PROXY_KEY", "").strip()


def sec_url(u: str) -> str:
    if not PROXY:
        return u
    q = f"{PROXY}/?u={quote(u, safe='')}"
    if PROXY_KEY:
        q += f"&k={quote(PROXY_KEY, safe='')}"
    return q


def sget(url: str, **kw):
    """SEC 요청 전용. 프록시가 설정돼 있으면 자동으로 경유시킨다."""
    return get(SEC, sec_url(url), **kw)


def log(*a): print(*a, file=sys.stderr, flush=True)


def get(session, url, tries=4, sleep=0.25, quiet=False, **kw):
    last = ""
    for i in range(tries):
        try:
            ...
            if r.status_code == 403:
                # SEC는 과속·UA 문제 시 IP를 약 10분간 차단한다. 길게 기다렸다 재시도.
                if i < tries - 1:
                    wait = 90 * (i + 1)
                    if not quiet:
                        log(f"  · SEC 차단(403) — {wait}초 대기 후 재시도 {i+2}/{tries}")
                    time.sleep(wait)
                    continue
                break
...
LOOKUP_CACHE = CACHE / "cik-lookup-data.txt"
_INDEX: list[tuple[str, str, int]] | None = None   # (정규화명, 원본명, CIK)


def cik_index() -> list[tuple[str, str, int]]:
    """SEC 공식 '전체 CIK·명칭' 파일을 받아 색인으로 만든다.
    형식은 한 줄에 `COMPANY NAME:0001234567:` 이며 약 90만 줄이다.
    구 browse-edgar 회사검색이 차단되어 이 방식으로 대체했다."""
    global _INDEX
    if _INDEX is not None:
        return _INDEX
    raw = ""
    if LOOKUP_CACHE.exists():
        age = time.time() - LOOKUP_CACHE.stat().st_mtime
        if age < 30 * 86400:
            raw = LOOKUP_CACHE.read_text(encoding="utf-8", errors="ignore")
    if not raw:
        log("SEC 전체 CIK 목록 내려받는 중 (13MB, 최초 1회)…")
        r = sget(LOOKUP_URL, tries=3, sleep=0.3)
        if not r:
            log("  ! CIK 목록 다운로드 실패")
            _INDEX = []
            return _INDEX
        raw = r.content.decode("utf-8", "ignore")
        LOOKUP_CACHE.write_text(raw, encoding="utf-8")
    idx = []
    for line in raw.splitlines():
        parts = line.rstrip(":").rsplit(":", 1)
        if len(parts) != 2 or not parts[1].isdigit():
            continue
        idx.append((norm(parts[0]), parts[0], int(parts[1])))
    _INDEX = idx
    log(f"CIK 색인 {len(idx):,}건 준비 완료")
    return idx


def has_13f(cik: int) -> bool:
    return len(quarters(cik, limit=1)) > 0


def candidates(query: str, limit: int = 8) -> list[tuple[str, int, float]]:
    """정규화 이름 기준 후보를 점수순으로. 완전일치 > 접두일치 > 부분포함 > 유사도."""
    q = norm(query)
    if not q:
        return []
    exact, prefix, contains = [], [], []
    for nkey, name, cik in cik_index():
        if nkey == q:
            exact.append((name, cik, 1.0))
        elif nkey.startswith(q):
            prefix.append((name, cik, 0.95 - min(len(nkey) - len(q), 40) / 400))
        elif q in nkey:
            contains.append((name, cik, 0.85 - min(len(nkey) - len(q), 40) / 400))
    pool = exact + sorted(prefix, key=lambda x: -x[2]) + sorted(contains, key=lambda x: -x[2])
    if not pool:  # 철자가 조금 다른 경우만 전체 유사도 비교 (비용이 크므로 최후수단)
        scored = [(name, cik, difflib.SequenceMatcher(None, q, nkey).ratio())
                  for nkey, name, cik in cik_index()
                  if abs(len(nkey) - len(q)) <= 12 and nkey[:4] == q[:4]]
        pool = sorted(scored, key=lambda x: -x[2])[:limit]
    # 같은 CIK 중복 제거
    seen, out = set(), []
    for name, cik, sc in pool:
        if cik in seen:
            continue
        seen.add(cik)
        out.append((name, cik, sc))
        if len(out) >= limit:
            break
    return out
...
def resolve_cik(query: str) -> dict | None:
    """이름 → CIK. 후보 중 실제로 13F를 제출한 곳을 골라 확정한다.
    결과는 data/cik_map.json 에 저장되어 다음 실행부터 재검색하지 않는다."""
    mp = jload(CIKMAP, {}) or {}
    if query in mp:
    ...
    if not cands:
        mp[query] = None
        jsave(CIKMAP, mp)
        log(f"  ✗ '{query}' 이름을 EDGAR 목록에서 못 찾음")
        return None
    for name, cik, sc in cands:
        if has_13f(cik):
            rec = {"cik": cik, "edgar_name": name, "score": round(sc, 2)}
            mp[query] = rec
            jsave(CIKMAP, mp)
            log(f"  · '{query}' → {name} (CIK {cik})")
            return rec
    mp[query] = None
    jsave(CIKMAP, mp)
    log(f"  ✗ '{query}' 후보 {len(cands)}곳 모두 13F 미제출 "
        f"(최상위: {cands[0][0]})")
    return None
...
# ════════════════════════════════════════════════════════ EDGAR
def submissions(cik):
    r = sget(f"https://data.sec.gov/submissions/CIK{cik:010d}.json")
...
def files_in(cik, acc):
    r = sget(adir(cik, acc) + "/index.json")
...
def parse_13f(cik, acc):
    ...
    r = sget(f"{adir(cik, acc)}/{xs[0]}", quiet=True)
    if not r: return {}
...
def load_prices(tickers, start):
    ...
    for t in dict.fromkeys(tickers):
        ...
        r = get(WEB, f"https://stooq.com/q/d/l/?s={sym}&i=d", tries=2, sleep=0.25, quiet=True)
        px = {}
...
def parse_13dg(cik, acc, primary):
    issuer = pct = ""
    r = sget(f"{adir(cik, acc)}/{primary}") if primary else None
    if r:
        try:
...
def preflight() -> bool:
    """SEC 접속 가능 여부를 먼저 확인한다. 서버 IP가 차단되면 여기서 사유가 드러난다."""
    log("=== SEC 접속 점검 ===")
    ua = SEC.headers.get("User-Agent", "")
    if "@" in ua:
        name, _, dom = ua.partition("@")
        shown = f"{name[:14]}…@{dom[:3]}…"
    else:
        shown = ua[:20]
    log(f"  보내는 User-Agent: {shown}  (길이 {len(ua)}자)")
    if PROXY:
        log(f"  중계 서버 사용: {PROXY[:44]}…  (키 {'있음' if PROXY_KEY else '없음'})")
    else:
        log("  중계 서버: 미사용 — GitHub에서 직접 접속하면 SEC가 403으로 막습니다")
    if len(ua.split()) < 2:
        log("  ! 형식 경고: '이름 이메일' 처럼 공백으로 구분된 두 부분이어야 합니다")
    ok = True
    r = sget("https://data.sec.gov/submissions/CIK0001067983.json", tries=2)
    if r:
        try: nm = r.json().get("name", "?")
        except Exception: nm = "?"
        log(f"  OK 공시 API 정상 ({nm})")
    else:
        log("  FAIL 공시 API 접속 불가"); ok = False
    r = sget(LOOKUP_URL, tries=2)
    if r:
        log(f"  OK CIK 명부 정상 ({len(r.content)/1e6:.1f}MB)")
        LOOKUP_CACHE.parent.mkdir(parents=True, exist_ok=True)
        LOOKUP_CACHE.write_text(r.content.decode("utf-8", "ignore"), encoding="utf-8")
    else:
        log("  FAIL CIK 명부 접속 불가"); ok = False
    if not ok:
        log("")
        if PROXY:
            log("중계 서버를 거쳤는데도 실패했습니다. 확인할 것:")
            log("  1) SEC_PROXY 주소가 정확한지 (끝에 / 없이, https:// 포함)")
            log("  2) SEC_PROXY_KEY 가 Worker의 TOKEN 값과 같은지")
            log("  3) 브라우저에서 Worker 주소를 직접 열어 JSON이 나오는지")
        else:
            log("SEC가 GitHub 서버 IP를 차단하고 있습니다. 중계 서버(SEC_PROXY)를 설정하세요.")
    return ok


def main():
    if not preflight():
        sys.exit("SEC 접속 실패로 중단합니다.")
    ...
    if not inst and not ppl:
        sys.exit("랭킹을 하나도 만들지 못했습니다. 위 로그에서 '못 찾음' 사유를 확인하고 "
                 "data/cik_map.json 의 해당 항목을 지운 뒤 다시 실행하세요.")
