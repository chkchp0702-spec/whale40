...
def holdings(fid: str, debug: bool = False) -> list[dict]:
    ...
    for c in cands:
        ...
        if c in seen_c:
            continue
        seen_c.add(c)
        if fid in c or "13f" in c.lower() or "data" in c.lower():
            ordered.append(c)
    if debug:
        log(f"    [debug] 데이터 주소 후보 {len(ordered)}개: {ordered[:6]}")

    for c in ordered[:8]:
        url = c if c.startswith("http") else BASE + (c if c.startswith("/") else "/" + c)
        d = get(url, quiet=True)
        tried.append(f"{c} → {'응답' if d else '없음'}")
        if not d:
            continue
        try:
            j = d.json()
        ...
        if rows:
            if debug:
                log(f"    [debug] 데이터 주소 확정: {c}  ({len(rows)}행)")
            break
