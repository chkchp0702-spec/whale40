...
def render(inst, ppl, hits, delta, agg, hist, details=None):
    today = date.today().isoformat()
    L = [f"# 고래 40 · {today}", "",
    ...
    if con:
        L += ["## 🤝 여러 곳이 함께 보유한 종목", "",
              "| 종목 | 보유 | 합계 주식수 | 합계 평가액 | 편입 | 편출 |", "|---|---|---|---|---|---|"]
        for a in con:
            L.append(f"| {stock_label(a['sym'], a['name'])} | {a['n_hold']}곳 | "
    ...
    for direction, title in (("in", "📈 편입이 몰린 종목"), ("out", "📉 편출이 몰린 종목")):
        ...
        if not rows_f:
            continue
        L += [f"## {title}", "", "| 종목 | 편입 | 편출 | 순증감 | 보유 | 합계 평가액 |",
              "|---|---|---|---|---|---|"]
        for a in rows_f:
            L.append(f"| {stock_label(a['sym'], a['name'])} | {a['n_in']} | {a['n_out']} | "
                     f"**{a['score']:+d}** | {a['n_hold']}곳 | {money(a['value'])} |")
        L.append("")
    ...
    if details:
        ...
        for d in details:
            ...
            if d["up"]:
                ...
                for x in d["up"]:
                    L.append(f"| {stock_label(x['sym'], x['issuer'])} | **+{x['pct']:.1f}%** | "
            ...
            if d["down"]:
                ...
                for x in d["down"]:
                    ...
                    L.append(f"| {stock_label(x['sym'], x['issuer'])}{tag} | **-{x['pct']:.1f}%** | "
                             f"{x['weight']:.2f}% | {x['prev_shares']:,.0f} → {x['shares']:,.0f} |")
                L.append("")
            if d.get("holdings"):
...
def make_pdf(path, inst, ppl, hits, delta, agg, hist, details=None):
    ...
    if con:
        ...
        data = [["종목", "보유 펀드", "합계 주식수", "합계 평가액", "편입", "편출"]]
        for a in con:
            data.append([stock_label(a["sym"], a["name"])[:26], f"{a['n_hold']}곳",
                         f"{a['shares']:,.0f}", money(a["value"]),
                         str(a["n_in"]), str(a["n_out"])])
        story.append(tbl(data, [52 * mm, 18 * mm, 30 * mm, 27 * mm, 14 * mm, 14 * mm], (2, 3, 4, 5)))
    ...
    for direction, title in (("in", "편입이 몰린 종목"), ("out", "편출이 몰린 종목")):
        ...
        data = [["종목", "편입", "편출", "순증감", "보유", "합계 평가액"]]
        for a in rows:
            data.append([stock_label(a["sym"], a["name"])[:26], str(a["n_in"]), str(a["n_out"]),
                         f"{a['score']:+d}", f"{a['n_hold']}곳", money(a["value"])])
        story.append(tbl(data, [52 * mm, 14 * mm, 14 * mm, 18 * mm, 16 * mm, 27 * mm], (1, 2, 3, 4, 5)))
    ...
    for rows, gl in ((inst, "기관"), (ppl, "유명인")):
        ...
        data = [["#", "이름", "1년", "최신", "상위 보유"]]
        for r in rows[:TOP_N]:
            data.append([str(r["rank"]), r["name"][:24], f"{r['ret_1y']:+.1f}%",
                         r["quarter"], ", ".join(r["top"])[:52]])
        story.append(tbl(data, [8 * mm, 42 * mm, 17 * mm, 19 * mm, 74 * mm], (2,)))
            ...
            if d["new"]:
                ...
                for x in d["new"][:12]:
                    data.append([stock_label(x["sym"], x["issuer"])[:24], f"{x['weight']:.2f}%",
                                 f"{x['shares']:,.0f}", money(x["value"])])
                story.append(tbl(data, [58 * mm, 18 * mm, 32 * mm, 28 * mm], (1, 2, 3)))
            if d["up"]:
                data = [["늘린 종목", "증가율", "비중", "주식수 직전→현재"]]
                for x in d["up"][:12]:
                    data.append([stock_label(x["sym"], x["issuer"])[:24], f"+{x['pct']:.1f}%",
                                 f"{x['weight']:.2f}%",
                                 f"{x['prev_shares']:,.0f} → {x['shares']:,.0f}"])
                story.append(tbl(data, [50 * mm, 20 * mm, 18 * mm, 48 * mm], (1, 2, 3)))
            if d["down"]:
                data = [["줄인 종목", "감소율", "비중", "주식수 직전→현재"]]
                for x in d["down"][:12]:
                    nm = stock_label(x["sym"], x["issuer"])[:20] + ("(전량)" if x.get("sold_out") else "")
...
def summary(inst, ppl, hits, delta, agg):
...
def send_telegram(text, md_path, pdf_path=None):
...
def inspect():
    ...
    if f:
        ...
        if qs:
            ...
            for h in hs[:5]:
                log(f"    {stock_label(h['sym'], h['issuer'])[:26]:26} {h['value']:>14,.0f}천$ {h['shares']:>14,.0f}주")
...
def main():
    ...
    md = render(inst, ppl, hits, delta, agg, hist, details)
    today = date.today().isoformat()
    ...
    pdf = None
    try:
        pdf = make_pdf(REPORT / f"{today}.pdf", inst, ppl, hits, delta, agg, hist, details)
        if pdf:
    ...
    send_telegram(summary(inst, ppl, hits, delta, agg), REPORT / f"{today}.md", pdf)
