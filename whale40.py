...
def summary(inst, ppl, hits, delta, agg, prev=None):
    prev = prev or {}
...
def send_telegram(text, md_path, pdf_path=None, digest_path=None):
    ...
    for p, fname, mime, cap in (
            (digest_path, f"whale40-요약-{today}.pdf", "application/pdf", f"📌 한 장 요약 {today}"),
...
def main():
    ...
    try:
        ...
        n = sum(len(items[k]) for k in ("rank", "ret", "hold", "flow", "fresh", "filings"))
        digest = make_digest_pdf(REPORT / f"{today}-digest.pdf", items, prev)
        if digest:
            log(f"한 장 요약 생성: 변화 {n}건 → report/{today}-digest.pdf")
    except Exception as e:
        log(f"요약 PDF 실패(무시하고 계속): {type(e).__name__}: {e}")

    print(md)
    log(f"\n완료 — 새 13F {len(hits)}건 → report/{today}.md")
    send_telegram(summary(inst, ppl, hits, delta, agg, prev),
                  REPORT / f"{today}.md", pdf, digest)
    jsave(DAYFILE, day_state(inst, ppl, agg, details))
